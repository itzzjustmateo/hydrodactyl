...
export const createServerGroup = async (name: string, serverIds?: number[]): Promise<ServerGroup> => {
    return http.post('/api/client/server-groups', { name, server_ids: serverIds }).then(({ data }) => ({
        ...data.attributes,
    }));
};
...
export const updateServerGroup = async (
    id: number,
    data: { name?: string; sort_order?: number; is_collapsed?: boolean },
): Promise<ServerGroup> => {
